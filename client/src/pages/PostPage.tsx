...
const PostPage: React.FC<PostPageProps> = ({ 
  ...
  setError('Post could not be loaded. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  setError('Post could not be deleted. Please try again later.');
      setIsDeleting(false);
    }
  };
  ...
  const createSanitizedHTML = (content: string) => {
    console.log({
      __html: DOMPurify.sanitize(content, {
        ALLOWED_TAGS: ['p', 'strong', 'em', 'br', 'ol', 'ul', 'li'],
        ALLOWED_ATTR: []
      })});
    return {
      __html: DOMPurify.sanitize(content, {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4">
      <Card className="w-full shadow-lg rounded-2xl">
        <CardHeader className="flex flex-col items-start gap-3">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Avatar name={post.author?.name.charAt(0)} size="sm">
              </Avatar>
        ...
        <CardBody>
          <div 
            className="prose max-w-none leading-relaxed text-default-700"
